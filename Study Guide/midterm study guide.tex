...
\]
\textbf{Important:} independence is \emph{not} required.
...
\[
    \Var(X+Y)=\Var(X)+\Var(Y).
\]
Without independence, \emph{do not} separate the variance like that.

\subsection{Indicator random variables}
For an event $A$, let $I_A = \1\{A\}$. Then
\[
    \E[I_A]=\Prob(A).
\]
This is one of the most useful tricks in the course.
...
\]
Law of total probability, for disjoint events $B_1,\dots,B_n$ forming a partition:
\[
...
Let $X\sim \Bern(p)$ with support $\{0,1\}$.

\textbf{PMF}
\[
    \Prob(X=x)=p^x(1-p)^{1-x}, \qquad x\in\{0,1\}.
\]
Equivalent form:
\[
...
Let $X\sim \Bin(n,p)$, the number of successes in $n$ independent Bernoulli$(p)$ trials.
...
Let $X\sim \mathrm{Hypergeo}(N,K,n)$, where a population has size $N$, with $K$ successes, and we draw $n$ items \emph{without replacement}. Then $X$ is the number of successes drawn.
...
\section{What you may want to add next}
Depending on exactly which strategic practice and homework sets you have finished, you may also want a short section on:
\begin{itemize}[leftmargin=1.5em]
    \item counting formulas and sampling table,
    \item independence of events and random variables,
    \item the tail-sum formula for nonnegative integer-valued random variables,
    \item Poisson approximation,
    \item common stories for each distribution,
    \item favorite tricks from your own solved strategic practice sets.
\end{itemize}

\section{One-page memory list}
\begin{align*}
\E[aX+b] &= a\E[X]+b,\\
\Var(X) &= \E[X^2]-(\E[X])^2,\\
\Var(aX+b) &= a^2\Var(X),\\
